...
# Descripción: Análisis de muestras, Pareto, combinaciones y estadística por variable
...
if st.button("🚀 Empezar análisis"):
    ...
    with c1:
        ...
        for i,v in enumerate(df_cnt['Muestras']):
            ax.text(i, v*1.01, str(v), ha='center')
    ...
    with c3:
        ...
        for i,v in enumerate(cnt2.values):
            ax2.text(i, v*1.01, str(v), ha='center')
    ...
    with c4:
        ...
        for i,v in enumerate(yearly.values):
            ax_yr.text(i, v*1.01, str(v), ha='center')
        ax_yr.set_xlabel('Año')
        ax_yr.set_ylabel('Cantidad de muestras')
        fig_yr.tight_layout()
    ...
    with c5:
        ...
        ax3.invert_yaxis()
        ax3.set_xlabel('Número de Alertas')
        for i,v in enumerate(top.values):
            ax3.text(v*1.01, i, str(v), va='center')
        cum = top.cumsum() / top.sum() * 100
        ax3_line = ax3.twiny()
        ax3_line.plot(cum.values, range(len(cum)), '-o', color='black')
        ax3_line.set_xlabel('% acumulado')
        for i,p in enumerate(cum):
            ax3_line.text(p+1, i, f"{p:.0f}%", va='center')
        fig3.tight_layout()
    ...
    with c6:
        ...
        if topc.empty:
            ...
            ax4_line = ax4.twiny()
            ax4_line.plot(cumc.values, range(len(cumc)), '-o', color='black')
            ax4_line.set_xlabel('% acumulado')
            for i,p in enumerate(cumc):
